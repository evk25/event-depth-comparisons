...
class StripPoolingAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gate = self.gate_conv(h_strip + w_strip)
        return x * gate
...
# Decoder
# =============================================================================
class EventDepthDecoder(nn.Module):
    def __init__(
        self,
        enc_dims: List[int],
        half_ch: int,
...
# Encoder
# =============================================================================
class EventDepthEncoder(nn.Module):
    def __init__(
        self,
        in_ch: int,
        dims: List[int],
...
# =============================================================================
# MAIN MODEL - ADAPTED FOR EVENT CAMERAS
# =============================================================================
class EventDepth(nn.Module):
    """
    EventDepth for event-based depth estimation.
    Adapted from original RGB model to accept 2-channel event input.
    """
    def __init__(
        self,
        variant: str = 'base',
        ...
        self.encoder = EventDepthEncoder(
            in_ch      = in_channels,
            dims       = cfg['dims'],
            depths     = cfg['depths'],
        ...
        self.decoder = EventDepthDecoder(
            enc_dims    = cfg['dims'],
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        H, W = x.shape[2:]
        s_half, enc_feats = self.encoder(x)
        return self.decoder(s_half, enc_feats, (H, W))

    def fuse_for_inference(self) -> 'EventDepth':
        self.eval()
        self.encoder.fuse()
        self.decoder.fuse()
        return self
    ...
    def print_model_summary(self):
        ...
        print(f"\n{'='*60}")
        print(f"EventDepth-{self.variant.upper()} (Event-Based)")
        print(f"{'='*60}")
...
def create_model(variant: str = 'base', in_channels: int = 2, **kwargs) -> EventDepth:
    """Create a EventDepth model."""
    variant = variant.lower().replace('-', '_').replace('zip_', '').replace('depth_', '')
    if variant not in MODEL_CONFIGS:
        variant = 'base'
    return EventDepth(variant=variant, in_channels=in_channels, **kwargs)
